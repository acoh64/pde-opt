...
def l2_rel_err(numeric, symbolic):
    numeric = np.asarray(numeric); symbolic = np.asarray(symbolic)
    return np.sqrt(np.sum((numeric - symbolic)**2)) / np.sqrt(np.sum(symbolic**2))

def test_convergence(numeric: Type[BaseEquation], symbolic: Type[BaseSymbolicEquation], numeric_args, symbolic_args, Ns, L):
