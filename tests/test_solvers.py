...
def test_1d_cahn_hilliard():
    ...
    np.testing.assert_allclose(
        solution.ys[-1].squeeze()[Nx // 4 : 3 * Nx // 4],
        analytic_solution[Nx // 4 : 3 * Nx // 4],
        rtol=1e-3,
        atol=1e-3,
    )
...
def test_2d_gross_pitaevskii():
    ...
    np.testing.assert_allclose(
        tf_density,
        density(solution.ys[-1][..., 0] + 1j * solution.ys[-1][..., 1]),
        rtol=1e-3,
        atol=1e-3,
    )
...
def test_1d_cahn_hilliard_pde_model():
    ...
    np.testing.assert_allclose(
        solution[-1].squeeze()[Nx // 4 : 3 * Nx // 4],
        analytic_solution[Nx // 4 : 3 * Nx // 4],
        rtol=1e-3,
        atol=1e-3,
    )
...
def test_1d_allen_cahn_pde_model():
    ...
    model = PDEModel(
        equation_type=AllenCahn2DPeriodic, domain=domain, solver_type=diffrax.Tsit5
    )
...
def test_2d_gross_pitaevskii_pde_model():
    ...
    pde_model = PDEModel(
        equation_type=GPE2DTSControl, domain=domain_, solver_type=StrangSplitting
    )
    ...
    np.testing.assert_allclose(
        tf_density,
        density(solution[-1][..., 0] + 1j * solution[-1][..., 1]),
        rtol=1e-3,
        atol=1e-3,
    )
